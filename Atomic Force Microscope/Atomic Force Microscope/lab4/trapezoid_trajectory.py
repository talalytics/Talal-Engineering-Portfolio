...
def trap_traj(a_max,v_max,x_end,f_s):
    ...
    while(True):
        ...
        if x2[i] == 0 and abs(x1[i]) < tol:
            break
    disp = [i + x_end for i in x1] # convert to displacement reading instead of tracking error
    velo = x2
    acc = a
    vec = np.array([disp,velo,acc]) # x1, x2, and acceleration points
    return vec
...
def main():
    ...
    np.savetxt('trajectory_disp.txt', disp, delimiter = '\t')
